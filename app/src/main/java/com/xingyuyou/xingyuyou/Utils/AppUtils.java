...
public class AppUtils {
    ...
    public static class AppSimpleInfo {
         ...
         * @param icon        图标
         * @param appSize     大小
         * @param versionName 版本码
         */
        public AppSimpleInfo(String name, Drawable icon, String appSize, String versionName) {
            this.setName(name);
            this.setIcon(icon);
    ...
    /**
     * 得到简单AppInfo的Bean(图标、应用名称、版本、大小)
     *
     * @param pm 包的管理
     * @param pi 包的信息
     * @return AppInfo类
     */
    private static AppSimpleInfo getSimpleBean(Context context, PackageManager pm, PackageInfo pi) {
        if (pm == null || pi == null) return null;
        ApplicationInfo ai = pi.applicationInfo;
        String packageName = pi.packageName;
        ...
        Drawable icon = ai.loadIcon(pm);
        String versionName = pi.versionName;
        return new AppSimpleInfo(name, icon, packageName, versionName);
    }
    ...
    /**
     * 获取所有已安装App简单信息
     * <p>{@link #getBean(PackageManager, PackageInfo)}（名称，图标，大小，版本号）</p>
     * <p>依赖上面的getBean方法</p>
     *
     * @param context 上下文
     * @return 所有已安装的AppInfo列表
     */
    public static List<AppSimpleInfo> getAppsSimpleInfo(Context context) {
        List<AppSimpleInfo> list = new ArrayList<>();
        PackageManager pm = context.getPackageManager();
        // 获取系统中安装的所有软件信息
        List<PackageInfo> installedPackages = pm.getInstalledPackages(0);
        for (PackageInfo pi : installedPackages) {
            AppSimpleInfo ai = getSimpleBean(context, pm, pi);
            if (ai == null) continue;
            list.add(ai);
        }
        return list;
     ...
     * <p>{@link #getBean(PackageManager, PackageInfo)}（名称，图标，包名，包路径，版本号，版本Code，否系统应用）</p>
     * <p>依赖上面的getBean方法</p>
     *
     * @param context 上下文
     * @return 所有已安装的AppInfo列表
                ...
                new IPackageStatsObserver.Stub() {
                    ...
                    public void onGetStatsCompleted(PackageStats pStats, boolean succeeded) throws RemoteException {
                      ...
                      /*  Toast.makeText(context, "缓存大小=" + Formatter.formatFileSize(context, pStats.cacheSize) +
                        ...
                        Log.e("daxiao", Formatter.formatFileSize(context, pStats.codeSize));
                        // 遍历一次消息队列，弹出Toast
                        // Looper.loop();
                    }
                });
